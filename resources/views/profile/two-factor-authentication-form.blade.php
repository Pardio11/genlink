<x-action-section>
    ...
    <x-slot name="description">
        {{ __('Añada una capa extra de seguridad con la autenticación en dos pasos.') }}
    </x-slot>
        ...
        <h3 class="text-lg font-medium text-gray-900">
            @if ($this->enabled)
                @if ($showingConfirmation)
                    {{ __('Termine de habilitar su autenticación en dos pasos') }}
                @else
                    {{ __('Su autenticación ha sido activada.') }}
                @endif
            @else
                {{ __('No está activada su autenticación.') }}
            @endif
        </h3>
